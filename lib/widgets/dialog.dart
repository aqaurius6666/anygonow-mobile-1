...
class CustomDialog {
  ...
  void show([optionData]) {
    ...
    var height = optionData != null ? optionData["height"] : null;
    if (type == "SUCCESS") {
      ...
      alert = alertDialogFailed(context, message, height);
    } else {
...
AlertDialog alertDialogFailed(context, message, height) {
  return AlertDialog(
    content: Container(
      width: getWidth(343),
      height: getHeight(height ?? 253),
      child: Column(
        children: [
          Container(
            height: getHeight(60),
